...
<body>
  <div class="wrapper">
    ...
    <div class="add-to-cart-loader">
      <img src="{{ asset('resources/assets/images/ajax-loader.gif') }}" id="img-load" />
      <div class="cart-updating-msg">{{ trans('frontend.cart_updating_msg') }}</div>
    </div>
  </div>
  
  <input type="hidden" name="frontend_all_msg_with_localization" id="frontend_all_msg_with_localization" value="{{ htmlspecialchars( $frontend_js_localization ) }}">  
  ...
  <script type="text/javascript">
    ...
    $(document).ready(function(){
      ...
      if($('#subscriptions_modal').length>0){
        @if($subscriptions_data['subscription_visibility'] == true && !$is_subscribe_cookie_exists && (!empty(get_current_page_name())  && count($subscriptions_data['popup_display_page']) > 0 && in_array(get_current_page_name(), $subscriptions_data['popup_display_page'])))
